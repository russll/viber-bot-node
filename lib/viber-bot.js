...
const REQUIRED_CONFIGURATION_FIELDS = ["authToken", "name", "avatar", "server"];
const SUBSCRIBED_EVENTS = ["subscribed", "unsubscribed", "conversation_opened", "message", "delivered", "seen"];
const API_URL = "https://chatapi.viber.com/pa";

function ViberBot(logger, configuration) {
	if (!configuration) {
	...
	this._logger = logger;
	this._client = new ViberClient(logger, this, API_URL, SUBSCRIBED_EVENTS);
	this._middleware = new Middleware(logger, new MessageValidator(logger, this.authToken), configuration.server);
	this._messageFactory = new MessageFactory(logger);
	this._regexMatcherRouter = new RegexMatcherRouter(logger);
...
ViberBot.prototype._getMissingFieldsInConfiguration = function(configuration) {
	return _.difference(Object.keys(configuration), REQUIRED_CONFIGURATION_FIELDS);
};
